...
export const drawback=(size,cns,angle,img)=>
{
  cns.save()
  cns.clearRect(0, 0, size, 800);
  cns.beginPath();
  cns.fillStyle = '#E3061E';
  cns.moveTo(size/2,20)
  cns.lineTo(size/2-20,80)
  cns.lineTo(size/2,90)
  cns.lineTo(size/2+20,80)
  cns.closePath();
  cns.fill();

  cns.beginPath();
  cns.fillStyle = '#3D57CF';
  cns.moveTo(size/2,780)
  cns.lineTo(size/2-20,720)
  cns.lineTo(size/2,710)
  cns.lineTo(size/2+20,720)
  cns.closePath();
  cns.fill();

  cns.translate(size/2,400)
  cns.rotate(angle)
  console.log(angle)

  
  img.src="../../assets/images/zhinan.png"
  img.onload = () => {
    cns.drawImage(img,-size/2,-size/2,600,600)
    cns.restore()
  }

  // cns.translate(size / 2, size / 2);
  ...
  //   }
  //   cns.beginPath();
  //   cns.moveTo(0, -size*3/8 );
  //   cns.lineTo(0, -size/3);
  //   cns.stroke();
  //   cns.fill();
  //   cns.closePath();
  //   cns.rotate(Math.PI/18)
  //}
}
...
const indicator = (size, cns, clockd) => {
  ...
  for (let a = 0; a < 36; a++) {
    ...
    cns.beginPath();
    cns.moveTo(calc * (size / 3) + (size / 2), y * (size / 3) + (size / 2));
    cns.lineTo(calc * (size / 3.25 - ekstra) + (size / 2), y * (size / 3.25 - ekstra) + (size / 2));
    cns.stroke();
    cns.fill();
    cns.closePath();
  }
